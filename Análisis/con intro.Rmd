...
set.seed(1)
boot_3000=as.svrepdesign(design=ps_pps, type='subbootstrap', replicates=3000)

dominios_3000 = as.data.frame(svyby(~cant_personas, ~pobre,boot_3000,svytotal)) %>% 
  rename(.,Cantidad_personas=cant_personas,SE=se,Pobre=pobre)%>%
  mutate(.,Pobre=ifelse(Pobre==1,"Pobre","No pobre"),
                        "2.5%"=Cantidad_personas-1.96*SE/sqrt(nrow(S)),
                        "97.5%"=Cantidad_personas+1.96*SE/sqrt(nrow(S))
...
poblacion_pobre = U  %>% group_by(.,pobre) %>% summarise(Freq = n())
