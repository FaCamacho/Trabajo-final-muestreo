...
tibble(est= te_boot$replicates) %>% ggplot()+
                                    geom_histogram(aes(x=est),bins=20,fill='orange3',color='white') +
                                    theme_minimal() +
...
-Muy pocos hogares tienen un ingreso per cápita menor o igual a 1650 o mayor a 1800. Esto se puede ver representado en las colas de la distribución.

-El ingreso per cápita se acumula entre los 1700 y 1750, tal como se ve en el centro del gráfico.

-La distribución va “pegando saltos” bastante notables para varios niveles del ingreso per cápita. Estos saltos se podrían interpretar como los cambios en el parámetro según el nivel socioeconómico de los hogares, mostrando que la diferencia entre un nivel y otro es muy marcada.
...
dominios%>%
  kable(.,caption="Tabla 5: Estimación del total de personas para los dominios de 'Pobre' y 'No pobre' con 1000 réplicas")%>%
  kable_styling(position = "center", latex_options = "HOLD_position")%>%
  kable_classic_2()
...
dominios_2000%>%
  kable(.,caption="Tabla 6: Estimación del total de personas para los dominios de 'Pobre' y 'No pobre' con 2000 replicas")%>%
  kable_styling(position = "center", latex_options = "HOLD_position")%>%
  kable_classic_2()
...
#personas_pobres = data.frame(pobre=c("No pobre","Pobre"),
 #                            Freq=c(sum(U$cant_personas)-sum(U$cant_personas*U$pobre),
  ...
  #mutate( Prop=Freq/sum(U$cant_personas))
...
post_estrati%>%
  kable(.,caption="Tabla 7: Estimación del total de personas para los dominios de 'Pobre' y 'No pobre' mediante postestratificación")%>%
  kable_styling(position = "center", latex_options = "HOLD_position")%>%
  kable_classic_2()
...
dominios$Estrategia = "1000 réplicas"
dominios_2000$Estrategia = "2000 réplicas"
