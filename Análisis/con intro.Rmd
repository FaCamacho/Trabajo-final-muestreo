...
ratios_jkn=svyratio(~ingreso_hog, ~cant_personas,jkn)
...
# Ajustar el tamaño del texto y aplicar un tema diferente
...
ggplot(dominios, aes(x = Pobre, y = Cantidad_personas)) +
  geom_point(size = 4, color = "black") +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0.2, color = "blue") +
  labs(title = "Cantidad de Personas por Grupo (Pobre y No pobre) con Intervalos de Confianza",
       x = "Grupo",
       y = "Cantidad de Personas") +
  theme_minimal()+
  theme(
...
# Ajustar el tamaño del texto y aplicar un tema diferente
ggplot(dominios_2000, aes(x = Pobre, y = Cantidad_personas, fill = Pobre)) +
  geom_bar(stat = "identity", position = position_dodge(), color = "black") +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0.2, position = position_dodge(0.9)) +
  labs(title = "Cantidad de Personas por Grupo (Pobre y No pobre)",
       x = "Grupo",
...
# Crear el gráfico de puntos con barras de error
ggplot(dominios_2000, aes(x = Pobre, y = Cantidad_personas)) +
  geom_point(size = 4, color = "black") +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0.2, color = "blue") +
  labs(title = "Cantidad de Personas por Grupo (Pobre y No pobre) con Intervalos de Confianza",
       x = "Grupo",
...
poblacion_pobre = data.frame(
  pobre = c(1, 0),
  Freq = c(sum(U$pobre*U$cant_personas), sum(U$cant_personas)-sum(U$pobre*U$cant_personas)) # Frecuencias conocidas en la población
)



ps_pps_postestr = postStratify(boot, strata = ~pobre, population = poblacion_pobre)


as.data.frame(svyby(~cant_personas, ~pobre, ps_pps_postestr, svytotal))

```

- Ajuste de pesos mediante calibración

```{r}


# Ajustar los pesos con calibración
ps_pps_calibrado <- calibrate(boot, formula = ~pobre, population = poblacion_pobre)

# Calcular las estimaciones usando el diseño calibrado
dominios_calibrated <- as.data.frame(svyby(~cant_personas, ~pobre, ps_pps_calibrado, svytotal))
```
